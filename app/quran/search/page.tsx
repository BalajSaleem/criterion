...
function SearchPageContent() {
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      const response = await fetch(
        `/quran/search/api?q=${encodeURIComponent(trimmedQuery.slice(0, 200))}`
      );
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    router.replace(`/quran/search?q=${encodeURIComponent(trimmedQuery)}`, {
                ...
                {["patience", "forgiveness", "prayer", "charity"].map((term) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      router.replace(`/quran/search?q=${encodeURIComponent(term)}`, {
                        scroll: false,
                      });
                      performSearch(term);
                    }}
